...
Interactive runs keep the step plan visible in a grid above the overall bar.
Pending steps show 0%, the active step animates, successful steps show 100%,
and failures stay marked. Small terminals fall back to the active step and
overall bar. Detailed output scrolls above the display and remains in the
private log. Administrator prompts stay in the foreground; non-interactive
runs retain plain output.
